...
trajectory, recording its model and tool calls through the Murmur gateway. A
wrapped graph therefore inherits Murmur's tracing, divergence, and per-step
diagnosis without any change to the graph itself.

The adapter is **observational**: the graph runs its own model and tools; Murmur
    ...
    def from_react_agent(
        ...
        try:
            ...
            raise ImportError(
                ...
                'pip install "murmur-harness[agents]"'
            ) from exc
