...
to pick the provider via ``MURMUR_MODEL_PROVIDER`` or ``--provider``.
...
class AnthropicPatchModel:
    ...
    def _ensure_client(self):
        ...
        try:
            ...
            raise BenchDependencyMissing(
                "anthropic is not installed; `pip install 'murmur-harness[bench]'`."
            ) from exc
...
class DeepSeekPatchModel:
    """DeepSeek v4 Pro with reasoning + thinking (override via ``MURMUR_MODEL``)."""
    ...
    def __init__(
        ...
        *,
        ...
        self.model = model or os.environ.get("MURMUR_MODEL") or self.DEFAULT_MODEL
    ...
    def _raise_if_reasoning_starved(self, *, choice: Any, text: str, output_tokens: int) -> None:
        ...
        raise BenchModelOutputError(
            ...
            "substantially; otherwise Murmur would submit an empty patch."
        )
