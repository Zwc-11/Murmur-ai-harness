...
def emit_trace(trace: Trace, port: TracePort) -> None:
    ...
    def walk(span: Span) -> None:
        ...
        attrs.setdefault("murmur.span.kind", span.kind)
        attrs.setdefault("murmur.duration_ms", round(span.duration_ms, 3))
...
def _emit_trace_metrics(trace: Trace, port: TracePort) -> None:
    for span in trace.spans:
        common = {
            "murmur.span.kind": span.kind,
            "murmur.trace.id": trace.trace_id,
            "murmur.run.id": trace.run_id,
            "murmur.trajectory.id": trace.trajectory_id,
        }
