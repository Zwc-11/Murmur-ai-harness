...
everything Murmur-specific lives under ``murmur.*``. Nothing is invented inside
...
def _trajectory_to_trace(events: list[Event], *, capture_content: bool, replay: bool) -> Trace:
        ...
        if event.type == EventType.TRAJECTORY_STARTED:
            ...
            step = add(
                Span(
                    ...
                    status="unset",
                    attributes={
                        "murmur.step.index": index,
                        "murmur.step.phase": phase,
                    },
                )
            )
            ...
            if capture_content and "content" in payload:
                attrs["murmur.model.content"] = payload["content"]
            last_leaf = add(
                Span(
            ...
            if capture_content and "args" in payload:
                for key, value in dict(payload["args"]).items():
                    tool_attrs[f"murmur.tool.arg.{key}"] = value
            open_tool = add(
                Span(
            ...
            if open_tool is not None:
                ...
                if errored:
                    ...
                    open_tool.attributes["murmur.tool.error_type"] = payload.get(
                        "error_type", "ToolError"
                    )
                    if capture_content:
                        open_tool.attributes["murmur.tool.error"] = payload["error"]
                open_tool = None
            cursor += duration
        elif event.type == EventType.CONTRACT_CHECK:
            accepted = bool(payload.get("accepted", False))
            attrs: dict[str, Any] = {
                "murmur.contract.result": "pass" if accepted else "fail",
            }
            if "step" in payload and payload.get("step") is not None:
                attrs["murmur.contract.step"] = int(payload["step"])
                if not accepted:
                    attrs["murmur.failure.step"] = int(payload["step"])
            for key in ("side", "field", "expected", "got"):
                if key in payload:
                    attrs[f"murmur.contract.{key}"] = payload[key]
            if payload.get("diagnostic_ids"):
                attrs["murmur.contract.diagnostic_ids"] = list(payload["diagnostic_ids"])
            add(
                Span(
    ...
    if outcome != "pass" and failure_class:
        if root is not None:
            root.attributes["murmur.failure.class"] = failure_class
            _stamp_failure_details(root.attributes, events)
        failing = last_leaf
        if failing is not None:
            failing.status = "error"
            failing.attributes["murmur.failure.class"] = failure_class
            _stamp_failure_details(failing.attributes, events)

    if replay:
        for span in spans:
            span.attributes["murmur.replay"] = True
    ...
    for event in events:
        if event.type == EventType.VERDICT:
            if event.payload.get("failure_step") is not None:
                attrs["murmur.failure.step"] = int(event.payload["failure_step"])
            if event.payload.get("failure_detail"):
                attrs["murmur.failure.detail"] = event.payload["failure_detail"]
            if event.payload.get("failure_confidence") is not None:
                attrs["murmur.failure.confidence"] = float(event.payload["failure_confidence"])
            return
