...
product called **Murmur** — an open-source reliability harness for coding agents.
...
4. Include the word "murmur" in the hero headline (case-insensitive) and an element \
...
def load_agent_task(name: str | None = None) -> TaskSpec:
    """Resolve the active agent task from *name*, ``MURMUR_TASK``, or YAML under ``tasks/``."""

    key = (name or os.environ.get("MURMUR_TASK") or "hard").strip().lower()
