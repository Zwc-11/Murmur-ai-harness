...
def render_fan_html(
    ...
    *,
    ...
    cards = "\n".join(
        [
            ...
            _metric_card(
                ...
                data_attrs=' data-murmur-failures-summary="1"' if failure_index else "",
            ),
        ]
    )
    ...
    body = (
        ...
        "</section>"
        f"{hud_shell_end()}"
        f'<script type="application/json" id="murmur-failure-data">{json.dumps(failure_index)}</script>'
...
def _fan_interaction_js() -> str:
    return r"""
(function () {
  const dataEl = document.getElementById("murmur-failure-data");
  ...
  document.querySelectorAll("[data-failure-class]").forEach((seg) => {
    seg.addEventListener("click", () => {
      ...
      html += "</ul>";
      murmurOpenModal("diagnosis · " + label, html);
    });
  });

  document.querySelectorAll("[data-murmur-cell]").forEach((cell) => {
    cell.addEventListener("click", (e) => {
      ...
      murmurOpenModal("overlay cell", html);
    });
  });

  const summary = document.querySelector("[data-murmur-failures-summary]");
  if (summary) {
    summary.addEventListener("click", () => {
      ...
      murmurOpenModal("failure breakdown", html);
    });
  }
})();
"""
...
def _overlay_section(overlay: DivergenceOverlay | None, trace_href: str | None) -> str:
    ...
    return (
        ...
        '<div class="hud-widget__bd t-skel" data-murmur-reveal>'
...
def _overlay_cell(
    ...
    data = (
        f'data-murmur-cell="1" data-state="{escape(cell.state)}" '
