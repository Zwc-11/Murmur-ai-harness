...
renders the three-region inspector specced in MURMUR_PHASE_1_BUILD.md —
...
def _build_template(data_json: str, run_label: str, *, n_traces: int) -> str:
    ...
    body = (
        f"{shell}"
        '<div id="app" class="trace-app t-skel" data-murmur-reveal>'
...
function spanModalHtml(t, s){{
  ...
  for (const [k,v] of entries) {{
    ...
    const err = (k==="status" && v==="error") || k==="murmur.failure.class";
...
function openSpanModal(){{
  ...
  murmurOpenModal("span · " + s.name, spanModalHtml(t, s));
}}
...
function renderInspector(){{
  ...
  entries.push(["status", s.status]);
  for(const [k,v] of entries){{
    const isErr = (k==="status" && v==="error") || k==="murmur.failure.class" ||
