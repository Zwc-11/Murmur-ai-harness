...
from chorus.report.agent_map_projector import build_preview_demos, project_agent_map
...
"""
    ...
    head = document_head(title="Chorus — agent map", extra_css=extra_css)
...
<section class="hud-widget am-stage-wrap">
  ...
  <div class="hud-widget__bd">
    <div id="agent-map-root" aria-label="Chorus agent operator map"></div>
  </div>
</section>
...
function loadDemo(key) {
  const demo = (window.CHORUS_AGENT_MAP.demos || {})[key];
  if (!demo) return;
  window.dispatchEvent(new CustomEvent("chorus-agent-map-load", { detail: demo }));
}
...
document.querySelectorAll(".am-op-chip-static").forEach((chip) => {
  chip.addEventListener("click", () => {
    window.dispatchEvent(
      new CustomEvent("chorus-agent-map-highlight", { detail: chip.dataset.op }),
    );
  });
});
"""
...
<script>
window.CHORUS_AGENT_MAP = {payload_json};
{chorus_ui_js()}
{preset_script}
</script>
