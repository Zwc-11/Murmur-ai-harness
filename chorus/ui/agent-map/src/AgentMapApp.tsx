...
function AgentMapCanvas({ data }: { data: AgentMapPayload }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("chorus-agent-map-load", onLoad);
    window.addEventListener("chorus-agent-map-highlight", onHighlight);
    return () => {
      window.removeEventListener("chorus-agent-map-load", onLoad);
      window.removeEventListener("chorus-agent-map-highlight", onHighlight);
    };
  }, [applyGraph]);
  ...
  const onNodeClick = useCallback((_e: MouseEvent, node: Node) => {
    ...
    window.chorusOpenModal?.(`module · ${d.label}`, html);
  }, []);
